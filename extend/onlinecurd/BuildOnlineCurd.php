...
class BuildOnlineCurd
{
   ...
   protected $allRelations=[];
    ...
    public function setOnly_api($only_api){
        $this->only_api=$only_api;
        return $this;
    }
    /**
     * 设置model放的的位置
    ...
    public function setRelations($relations)
    {
        ...
        foreach ($relations as $val) {
            $this->foreigns[$val['foreign_key']] =
            [ 
                'table_name'=>$val['table_name'],
                'selectpage_label'=>$val['selectpageLabel'] ?? $val['primary_key'],
            ];
        }
        return $this;
    }
    public function setOneToManyRelations($relations)
    {
        $this->oneToManyRelations = $relations;
        return $this;
    }
    public function setAllRelations($relations)
    {
...
EOD;
    ...
    public function checkPre(){
        foreach($this->allRelations as $k=>$v){
            $re_table_name = $this->getTableNameExpPrefix($v['table_name']);
                    // 初始化默认控制器名
            $reNodeArray = explode('_', $re_table_name);
            $reControllerFilename='';
            if (count($reNodeArray) == 1) {
                $reControllerFilename = ucfirst($reNodeArray[0]);
            } else {
                foreach ($reNodeArray as $k => $v) {
                    if ($k == 0) {
                        $reControllerFilename = "{$v}{$this->DS}";
                    } else {
                        $reControllerFilename .= ucfirst($v);
                    }
                }
            }
            $controllerFile = "{$this->rootDir}app{$this->DS}{$this->back_module}{$this->DS}controller{$this->DS}{$reControllerFilename}.php";
            if (!is_file($controllerFile)) {
                
                throw new FileException("请先生成子表：".$re_table_name);
            }
        }
        return $this;
    }
...
use util\Excel;';
    ...
    protected function renderModel()
    {
        ...
        foreach ($this->relations as $val) {
            ...
            $modelArray = explode($this->DS, $modelFilename);
            $modelName = array_pop($modelArray);
            $subPrimaryKey = $val['subPrimaryKey'] ?? 'id';
...
EOD;
     ...
     * 生成一对多页面
     */
    protected function buildOneToMoreHtml($fields,$foreign_key,$controllerUrl){
        $formData='';
        $data='';
        $coloumsJs='';
        $queryLength=0;
        ...
        $editFormList='';
        $requireJs='';
        $importHtml="import EditForm from './Edit'";
        $components="EditForm";

        
        //构建查询js
        $queryFormJs=<<<EOD
                $foreign_key: { value: undefined, op: '=' },
EOD;
        //props构建
        $props= <<<EOD
    props: {
...
$foreign_key: {
            handler(val, old) {
                if (val) {
                    this.queryParams.$foreign_key.value = val
                    this.initIndex();
                }
            },
            immediate: true,
...
EOD;
        ...
        foreach($fields as $key=>$val){
            ...
            $index_show = isset($val['index_show']) ? $val['index_show'] : false;
            $coloumsJsTemp =  $index_show ? $this->getColoumsJs($formType, $field, $comment, $dict) : '';
            $relation = '';
           
            $coloumsJs .= $coloumsJsTemp;
            $is_query= isset($val['is_query']) ? $val['is_query'] : false;
            if ($is_query) {
            ...
            $edit_show=isset($val['edit_show']) ? $val['edit_show'] : false;
            //edit表单
            if (in_array($field, ['create_time']) || in_array($field, $this->ignoreFields) || !$edit_show) {
                continue;
            }
...
EOD;
      ...
      EOD;
              ...
              $viewIndexValue = $this->replaceTemplate(
                  ...
                  [
                      ...
                      'data' => $data,
                      'showSearch'=>$queryLength >0 ? 'true' :'false',
                  ...
                  return [$viewEditValue,$viewIndexValue];
    ...
    protected function renderView()
    {
        ...
        $filePathPrefix="{$pathFront}{$this->DS}{$frontBase}{$this->DS}src{$this->DS}views{$this->DS}{$this->viewFilename}";
        $viewIndexFile = "{$filePathPrefix}{$this->DS}index.vue";

        // 编辑页面
        $viewEditFile = "{$filePathPrefix}{$this->DS}EditFormCom.vue";
        $editFormList = '';
        ...
        foreach ($this->fieldlist as $val) {
            ...
            $relation = '';
            $showField = '';
            if (isset($this->foreigns[$field]['table_name'])) {
                $templateFile = "view{$this->DS}module{$this->DS}selectPage";
                $subTable = $this->getControllerUrl($this->foreigns[$field]['table_name']);
                $relation = $subTable;
                $formType = "selectPage";
                $showField=isset($this->foreigns[$field]['selectpage_label']) ? $this->foreigns[$field]['selectpage_label'] : '';
            }
            $coloumsJs .= $coloumsJsTemp;
            if ($val['is_query']) {
                $templateFileQuery = $templateFile;
            ...
            //edit表单
            if (in_array($field, ['create_time']) || $val['is_primary'] || in_array($field, $this->ignoreFields)) {
                continue;
            }
            $editFormElement = $this->replaceTemplate(
                $this->getTemplate($templateFile),
...
EOD;
        ...
        foreach ($this->relations as $table => $tableVal) {
            ...
            foreach ($tableVal['fieldlist'] as $val) {
                ...
                //edit表单
                if (in_array($field, ['create_time']) || in_array($field, $this->ignoreFields)) {
                    continue;
                }
            }
        ...
        $oneToManyDialogHtml="";
        $btnHtml="";
        $importHtml="import EditForm from './EditFormCom'";
        $components="EditForm";
        $methods="";
        foreach($this->oneToManyRelations as $k=>$v){
            ...
            $reTableName='';
            $reControllerFilename="";
            foreach ($reNodeArray as $key => $val) {
                if ($key == 0) {
                    $reTableName = $val;
                    $reControllerFilename = "{$val}{$this->DS}";
                } else {
                    $reTableName .= ucfirst($val);
                    $reControllerFilename .= ucfirst($val);
                }
            }
            if (count($reNodeArray) == 1) {
                $reControllerFilename = ucfirst($reNodeArray[0]);
            }
            $reNodeArray = explode($this->DS, $reControllerFilename);
            $reformatArray = [];
            foreach ($reNodeArray as $vo) {
                $reformatArray[] = $this->humpToLine(lcfirst($vo));
            }
            $reControllerUrl = implode('.', $reformatArray);



            $visible=$reTableName."Show";
            $beforeClose="beforeClose".ucfirst($reTableName);
            $lcdialogCom=$reTableName."Com";
            $dialogCom=ucfirst($reTableName)."Com";

            $foreign_key=$v['foreign_key'];
            $primary_key=$v['primary_key'];
            $title=$v['remark'] ? $v['remark'] :"列表";
            $handle="handle".ucfirst($reTableName);
            /**
             * 列表弹框
             */
            $oneToManyDialogHtml .= <<<EOD
        <el-dialog width="70%" title="$title" :visible="$visible" :before-close="$beforeClose" destroy-on-close>
            <$dialogCom @close="$beforeClose" :api="api" :$foreign_key="$foreign_key" />
        </el-dialog>

EOD;
            //操作按钮
            $btnHtml.= <<<EOD
                            <el-button  class="btn-mar" size="mini" type="info" @click="$handle(scope.row)">$title</el-button>
            
EOD;
            //导入页面
            $importHtml.=<<<EOD

import $dialogCom from './$reTableName/list';
EOD;
            //components
...
EOD;
        ...
        $handle(row) {
            this.$visible = true
            this.$foreign_key = row.$primary_key
        },
...
EOD;
        ...
        //一对多页面生成
        list($oneManyEditHtml,$oneManyIndexHtml)=$this->buildOneToMoreHtml($v['fieldlist'],$foreign_key,$reControllerUrl);
        $oneManyViewIndexFile = "{$filePathPrefix}{$this->DS}{$reTableName}{$this->DS}list.vue";
        $oneManyViewEditFile = "{$filePathPrefix}{$this->DS}{$reTableName}{$this->DS}Edit.vue";

        $this->fileList[$oneManyViewIndexFile] = $oneManyIndexHtml;
        $this->fileList[$oneManyViewEditFile] = $oneManyEditHtml;
        }
...
EOD;
        ...
        $viewIndexValue = $this->replaceTemplate(
            ...
            [
                ...
                'showSearch'=>$queryLength >0 ? 'true' :'false',
                'expandHtml' => $expandHtml,
                'oneToManyDialogHtml'=>$oneToManyDialogHtml,
                'btnHtml'=>$btnHtml,
                'importHtml'=>$importHtml,
                'components'=>$components,
                'methods'=>$methods,
                'dialogMul'=>'',
                'created'=>"this.initIndex()",
                'props'=>'',
                'watch'=>'',
                'otherprop'=>'',
            ]);
    ...
    public function getSubTableRow($sub_table)
    {
        ...
        $data['subPrimaryKey'] = $subPrimaryKey;
        $data['fieldlist'] = $fieldlist;
        return $data;
    }
