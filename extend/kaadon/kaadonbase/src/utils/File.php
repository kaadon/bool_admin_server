...
namespace Kaadon\KaadonBase\utils;
...
class File
{
    ...
    public static function compresorPath(string $fromPath, string $toPath,string $filename, array $jump = []): bool
    {
        if (!is_dir($fromPath)){
            return false;
        }
        if (!is_dir($toPath)){
           self::dirMkdir($toPath);
        }
        $zipArchive = new ZipArchive;
        if ($zipArchive->open($toPath . $filename,$zipArchive::CREATE|$zipArchive::OVERWRITE) === true){
            if ($handle = opendir($fromPath)){
                // 添加目录中的所有文件
                while (false !== ($entry = readdir($handle))){
                    if ($entry != "." && $entry != ".." && !is_dir($fromPath . $entry)){
                        $zipArchive->addFile($fromPath . $entry,$entry);
                    }
                }
                closedir($handle);
            }
